...
export class RealisationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.realisationService
      ...
      .subscribe(
        (res: HttpResponse<IRealisation[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IRealisation): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRealisations(): void {
    this.eventSubscriber = this.eventManager.subscribe('realisationListModification', () => this.loadPage());
  }

  delete(realisation: IRealisation): void {
    const modalRef = this.modalService.open(RealisationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IRealisation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
